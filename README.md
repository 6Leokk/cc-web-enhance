...
- 增强版仓库：[`6Leokk/cc-web-enhance`](https://github.com/6Leokk/cc-web-enhance)
...
- **轻量运行时** — 后端依赖少，通过浏览器访问本机 Claude Code / Codex CLI
- **双 Agent 会话** — 在同一套 Web UI 中创建 Claude 或 Codex 会话
- **Agent 隔离视图** — 切换 Claude / Codex 时只显示对应会话、状态、设置和导入入口
- **Agent 独立设置** — Claude 保留模板化模型配置，Codex 独立配置路径、默认模型、模式和搜索选项
- **多会话管理** — 创建、切换、重命名、删除会话，删除时同步清除本地 Claude 历史记录
- **本地历史导入** — Claude 可导入 `~/.claude/projects/` 会话；Codex 可导入 `~/.codex/sessions/` rollout 历史
- **会话续接** — 基于 CLI 原生续接能力保持上下文，必要时可通过 SSH + `tmux attach -t claude` 重新接入
- **后台任务与通知** — 关闭浏览器后 Claude 进程继续运行，完成后支持 PushPlus / Telegram / Server酱 / 飞书机器人 / QQ（Qmsg）/ Bark 通知
- **进程持久化** — detached 子进程 + PID 文件，服务重启后可恢复运行中任务
- **多 API 切换** — 可配置多个 API 方案，一键切换，即时生效
- **开发者配置** — 保存 SSH 主机信息和 GitHub token，通过 `/ssh`、`/github` 快速管理远程主机和仓库
- **密码认证** — 自动生成初始密码、首次登录强制改密、Web UI 修改密码
- **增强稳定性** — 会话懒加载、刷新缓存、事件作用域隔离、Codex rollout 解析和多条回归脚本，降低刷新慢、串会话和状态栏滞后的风险
...
首次运行会在终端提示粘贴 ngrok authtoken，并可选填写固定 domain 和 Basic Auth。脚本会自动把 `.env` 切到 `CC_WEB_ACCESS_MODE=ngrok`、保持 `CC_WEB_HOST=127.0.0.1`、开启 `NGROK_AUTO_START=1`，然后启动 cc-web。后续再运行同一命令会复用 `.env` 里的配置。
...
推荐流程：启动后打开设置，选择本机、局域网、无公网下远程访问、公网服务器或 frp；当界面给出 QR/quick-login 链接时，可以扫码或复制链接登录。quick-login 使用 `/#pair=` fragment，不把配对 token 放进 query string；裸 public HTTP 默认禁用 quick-login。
...
| `CC_WEB_KILL_PORT_OCCUPANT` | 否 | `0` | 设为 `1` 时允许启动前自动结束占用端口的进程；默认关闭 |
