...
- 在输入框下方新增一行纯文字状态栏，当前格式为：
  - `模型 | 当前目录 | 已用 Token / 总上下文 | Git 分支 (+新增 -删除)`
- 最左侧新增模型类型显示，例如 `gpt-5.4`、`gpt-5.5`、`opus`、`sonnet`。
- 当前目录使用短路径展示；远程任务优先显示远端工作目录。
...
- 中间的“已用 Token”优先使用运行时真实 usage 数据。
- 当前版本中：
  - Codex 会话可显示 runtime/input token 使用量。
  - Claude 会话当前没有 runtime token 使用事件时，已用值显示为未知。
- “总上下文”只在有明确真值时显示：
