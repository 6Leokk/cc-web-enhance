const os = require('os');
const { recommendLanBindHost } = require('./access-network');
...
function resolveServerBindConfig(env = process.env, options = {}) {
  const hostValue = readEnvValue(env, 'CC_WEB_HOST', 'HOST');
  const portValue = readEnvValue(env, 'CC_WEB_PORT', 'PORT');
  let host = hostValue ? hostValue.value : DEFAULT_SERVER_HOST;
  const accessConfig = options.accessConfig || null;
  if (!hostValue && accessConfig?.mode === 'direct' && accessConfig?.directScope === 'lan') {
    const networkInterfaces = options.networkInterfaces || os.networkInterfaces();
    host = recommendLanBindHost(networkInterfaces) || DEFAULT_SERVER_HOST;
  }
  return {
    host,
