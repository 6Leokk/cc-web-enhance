    ...
    switch (event.type) {
	            ...
	            if (Array.isArray(update?.events)) {
	              for (const event of update.events) {
	                if (event.type === 'assistant_segment_start') {
	                  wsSend(entry.ws, { type: 'assistant_segment_start', prefix: event.prefix || '' });
	                } else if (event.type === 'text_delta') {
	                  wsSend(entry.ws, { type: 'text_delta', text: event.text || '' }, true);
	                }
	              }
	            }
	          }
      ...
      case 'turn.completed': {
        ...
        if (session && usage) {
          ...
          saveSession(session);
          wsSend(entry.ws, { type: 'usage', totalUsage: session.totalUsage, lastUsage: session.lastUsage }, true);
        }
        break;
      }
